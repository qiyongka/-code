# -*- coding: utf-8 -*-
"""
将fft中数据读出来一行一行的进行读取，并且存起来，就可以得到基频点和倍频点的增益
...
for folder in range(len(foldernames)):
    ...
    input_file = input_path +'/'+ foldernames[folder]+'_10fft.csv'            #输入fft后的结果
    output_file = output_path +'/' +foldernames[folder] + '_fft_point.csv'  #输出基频和倍频的增益
    ...
    for i in range(samples):
        ...
        for point in range(0,110,10):          
            #m = freq[int(point)]
            array[int(point/10)] = y[int(point)]
        print(array)
